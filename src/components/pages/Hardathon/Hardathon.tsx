...
import {
    hardathonPlaceholder,
    hardathonShortPlaceholder,
...
export const Hardathon = () => {
    ...
    const {data: fullEvent} = useQuery<Hardathons>({
        ...
        queryFn: () => fetch(`${AppConfig.apiUri}/api/v0${location}/1`).then((r) => r.json()),
    ...
    const handleSelect = (selectedIndex: number) => {
        if (selectedIndex < 0) setIndex(items.length - 1);
        else if (selectedIndex >= items.length) setIndex(0);
        else setIndex(selectedIndex);
    };
                ...
                <div className='board'>
                    ...
                    <div className={'box-hardathon d-inline-block'}>
                        <div className={'quote'}>
                            <p></p>
                            <p className={'author'}>
                                {fullEvent?.main_organizer_word}— главный организатор хардатона,
                                Валерий Бизиков{' '}
                            </p>
                        </div>
